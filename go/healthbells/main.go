...
var (
	healthStatus = &HealthStatus{&sync.Mutex{}, []CheckResult{}, 0}
	...
	for i := 0; i < len(urls); i++ {
		...
		if result.healthy {
			...
			if !config.Quiet || config.Interval < 0 {
				logger.Printf("💖 %s is up with status=%d", result.target, result.statusCode)
			}
		} else {
			msg := fmt.Sprintf("⚡ %s is down with status=%d", result.target, result.statusCode)
			/*
...
func checkUrl(url string, c chan CheckResult) {
	...
	// Create context with timeout for better cancellation support
	...
	if len(healthStatus.Results) < config.Histlen {
		healthStatus.Results = append(healthStatus.Results, CheckResult{})
	}

	// Use circular buffer - overwrite old data when full
	healthStatus.Results[healthStatus.pos] = checkResult
	healthStatus.pos = (healthStatus.pos + 1) % config.Histlen
}
...
<tbody>`)
	//now := time.Now()
	// Display results in reverse chronological order (circular buffer)
	for i := 0; i < len(healthStatus.Results); i++ {
		idx := (healthStatus.pos - 1 - i + len(healthStatus.Results)) % len(healthStatus.Results)
		if idx < 0 {
			idx += len(healthStatus.Results)
		}
		element := healthStatus.Results[idx]
