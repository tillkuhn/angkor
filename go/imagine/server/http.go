...
func NewHandler(s3Handler *s3.Handler, config *types.Config) *Handler {
	cache := ttlcache.New[string, string](
		...
		// if config.PresignExpiry is < 5 m, objects will be (probably) removed immediately on the first cleanup run
...
func (h *Handler) PostObject(w http.ResponseWriter, r *http.Request) {
	...
	// This looks also promising: https://golang.org/pkg/net/http/#DetectContentType DetectContentType
	...
	if strings.HasPrefix(contentType, ContentTypeJson) {
		...
		// if the request contains a filename, use this instead of url filename but append suffix if not present
