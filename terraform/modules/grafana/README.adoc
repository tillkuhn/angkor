...
It seems Grafana Cloud suspend Grafana Dashboards after they haven't been used for some time, which leads to 503 errors when the Provider wants to interact with the API, thus blocking tf plan resp. tf apply. See also
https://github.com/grafana/terraform-provider-grafana/issues/250[]
