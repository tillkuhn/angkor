...
1. See https://grafana.com/docs/grafana-cloud/monitor-infrastructure/integrations/integration-reference/integration-confluent-cloud/[Confluent Cloud integration for Grafana Cloud] and https://docs.confluent.io/cloud/current/monitoring/metrics-api.html#metrics-quick-start[Confluent Docs: Create an API key to authenticate to the Metrics API]
2. Service Account: ${env_id}-grafana-monitoring for metrics access
3. Role Binding: Granted MetricsReader role at the environment level
4. API Key: Dedicated API key for the monitoring service account
5. Output: Structured output with service account and API key details (marked as sensitive)
